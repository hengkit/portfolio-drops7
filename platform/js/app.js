...
var _marketing = {
    ...
    'newrelic'     :["New Relic","It’s about gaining actionable, real-time business insights from the billions of metrics your software is producing, including user click streams, mobile activity, end user experiences and transactions.<img src='img/newrelic-graph.png'>"],
    'indexserver'  :["Solr™","The popular, blazing fast open source enterprise search platform from the Apache Lucene project. Its major features include powerful full-text search, hit highlighting, faceted search, near real-time indexing, dynamic clustering, database integration, rich document (e.g., Word, PDF) handling, and geospatial search."],
  ...
  model: function(params){
    return new Ember.RSVP.Promise(function(resolve){
      ...
      if(App.env == 'enterprise'){
        target = install_dir + "enterprise.json";
      }
      else if(App.env == 'business'){
        target = install_dir + "business.json";
      }
      else if(App.env == 'personal'){
        target = install_dir + "personal.json";
      }

      $.get("/"+target+"?env="+params.env_id, function(d){
        console.log("$.get return", d);
        if(!App.mySite){
          console.log("Building a newish site.");
          App.mySite = new Site();
        }
        App.mySite.load(d.servers);
        console.log("site: ", App.mySite);
        resolve(App.mySite);
        if(App.poller){
...
App.InfobarView = Ember.View.extend({
  ...
  didInsertElement: function(){
    var self = this;
    console.log("View Inserted: App.InfobarView");
    infobarViewInstance = self;
  },
  open: function(){
    var self = this;
    //$("#infobarContainer").show("slide", {direction:"right"}, 500);
    $("#infobarContainer").addClass("emerge");
    $("#ember248").addClass("emerge");
  },
  close: function(){
    var self = this;
    //$("#infobarContainer").hide("slide", {direction:"right"}, 500);
...
App.InfobarContentView = Ember.View.extend({
  ...
  didInsertElement: function(){
    ...
    console.log("View Inserted: App.InfobarContentView");
  ...
  nodeChange : function(){
    var self = this;
    console.log("nodeChange in view!");
    ...
    if(node){
      ...
      var icoColor = "white";
    //  if(node.type == "appserver" || node.type == "indexserver" || node.type == "slavedbserver"){
    //    icoColor = "black";
    //  }
...
App.DiagramView = Ember.View.extend({
  ...
  didInsertElement: function(){
    var self = this;
    console.log("View Inserted: App.DiagramView");
